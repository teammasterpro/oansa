...
--
-- Servidor: localhost
-- Tiempo de generación: 08-10-2018 a las 14:03:14
...
-- Procedimientos
--
CREATE DEFINER=`root`@`localhost` PROCEDURE `ActulizarResponsabilidad` (`idTipoRes` TINYINT, `idLider` BIGINT, `fechaIni` DATE, `fechaF` DATE, `Igle` INT, `idRespo` BIGINT)  BEGIN
	#Routine body goes here...	update responsabilidad set responsabilidad.Lider=idLider,responsabilidad.FechaInicio=fechaIni,
responsabilidad.FechaFin=fechaF,responsabilidad.tipoResponsabilidad=idTipoRes,responsabilidad.Iglesia=Igle
where responsabilidad.idResponsabilidad=idRespo;

END$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `BorrarResponsabilidad` (`idRespo` BIGINT)  BEGIN
	#Routine body goes here...DELETE from responsabilidad where responsabilidad.idResponsabilidad=idRespo;

END$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `InsertPuntajes` (`Memor` SMALLINT, `Bibl` SMALLINT, `Uni` SMALLINT, `Cuot` SMALLINT, `Vis` SMALLINT, `Igle` INT)  BEGIN
	#Routine body goes here...insert into puntajejuegos(puntajejuegos.Memorizacion,puntajejuegos.Biblia,puntajejuegos.Uniforme,puntajejuegos.Cuota,puntajejuegos.Vsita,puntajejuegos.Fecha,puntajejuegos.estado,puntajejuegos.iglesia)
values(Memor,Bibl,Uni,Cuot,Vis,CURRENT_DATE,1,Igle);
END$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `InsertResponsabilidad` (`idTipoRes` TINYINT, `idLider` BIGINT, `fechaIni` DATE, `fechaF` DATE, `Igle` INT)  BEGIN
	#Routine body goes here...INSERT INTO responsabilidad(responsabilidad.Lider,responsabilidad.FechaInicio,responsabilidad.FechaFin,
responsabilidad.tipoResponsabilidad,responsabilidad.Iglesia)
values(idLider,fechaIni,fechaF,idTipoRes,Igle);
END$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `ListarDenomi` ()  BEGIN
	#Routine body goes here...SELECT * from denominacion;
END$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `ListarDepart` (`idPa` TINYINT)  BEGIN
	#Routine body goes here...select departamento.IdDepat,departamento.Nombre from departamento
where departamento.Pais=idPa;
END$$
...
where evento.Iglesia=Iglesia.idIglesia and evento.fechaEvento>=CURRENT_DATE
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `ListarIglesiaByBusqueda` (`idPa` TINYINT, `idDep` SMALLINT, `idMuni` SMALLINT, `idDeno` SMALLINT)  BEGIN
	#Routine body goes here...
SELECT iglesia.idIglesia,iglesia.Nombre from iglesia
where iglesia.Pais=idPa and iglesia.Departamento=IdDep
and iglesia.Municipio=IdMuni and iglesia.Denominacion=idDeno;
	
END$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `ListarMuni` (`idDep` SMALLINT)  BEGIN
	#Routine body goes here...select municipio.idMunicipio,municipio.Nombre from municipio
where municipio.Depart=idDep;
END$$
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `ListarPuntajes` (`Igle` INT)  BEGIN
	#Routine body goes here...select * from puntajejuegos
where puntajejuegos.estado=1 and puntajejuegos.iglesia=Igle;
END$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `QuitarPuntajes` (`idPunt` BIGINT)  BEGIN
	#Routine body goes here...UPDATE puntajejuegos set puntajejuegos.estado=0
where puntajejuegos.idPuntajes=idPunt and puntajejuegos.estado=0;
END$$
...
INSERT INTO `iglesia` (`idIglesia`, `Nombre`, `Pais`, `Departamento`, `Municipio`, `Direccion`, `Denominacion`) VALUES
(1, 'emanuel', 15, 20, 405, 'cra 39 #6n-106', 1);
...
INSERT INTO `lider` (`nDoc`, `tipoLider`, `fechaIngreso`) VALUES
(1065904453, 2, '2018-10-05 00:00:00.000');
...
INSERT INTO `persona` (`nDoc`, `tipoDoc`, `Nombre`, `fechaNac`, `Direccion`, `Telefono`, `Sexo`, `Estado`, `paisResidencia`, `departResidencia`, `muniResidencia`, `Iglesia`) VALUES
(1065904453, 1, 'CRISTIAN EDUARDO VILLALBA RABIA', '0000-00-00', 'CALLE 11 #15-13', 3172526468, 1, 1, 15, 20, 405, 1);
...
INSERT INTO `responsabilidad` (`idResponsabilidad`, `tipoResponsabilidad`, `Lider`, `FechaInicio`, `FechaFin`, `Iglesia`) VALUES
(1, 1, 1065904453, '2018-10-05', '2018-11-06', 1);
...
--
-- Estructura Stand-in para la vista `v_color`
...
--
-- Estructura Stand-in para la vista `v_denominacion`
-- (Véase abajo para la vista actual)
--
CREATE TABLE `v_denominacion` (
`idDeno` smallint(6)
,`Nombre` varchar(91)
);
...
--
-- Estructura Stand-in para la vista `v_estado`
-- (Véase abajo para la vista actual)
--
CREATE TABLE `v_estado` (
`idEstado` tinyint(3)
,`Nombre` varchar(12)
);
...
--
-- Estructura Stand-in para la vista `v_pais`
-- (Véase abajo para la vista actual)
--
CREATE TABLE `v_pais` (
`idPais` tinyint(3)
...
--
-- Estructura Stand-in para la vista `v_parentesco`
-- (Véase abajo para la vista actual)
--
CREATE TABLE `v_parentesco` (
`idparentesco` tinyint(4)
,`nombre` varchar(10)
);
...
--
-- Estructura Stand-in para la vista `v_rango`
-- (Véase abajo para la vista actual)
--
CREATE TABLE `v_rango` (
`idRango` tinyint(3)
,`Nombre` varchar(10)
);
...
--
-- Estructura Stand-in para la vista `v_sexo`
-- (Véase abajo para la vista actual)
--
CREATE TABLE `v_sexo` (
`idSexo` tinyint(3)
...
--
-- Estructura Stand-in para la vista `v_tipodoc`
-- (Véase abajo para la vista actual)
--
CREATE TABLE `v_tipodoc` (
`idTipoDoc` tinyint(3)
,`Nombre` char(20)
);
...
--
-- Estructura Stand-in para la vista `v_tipolider`
-- (Véase abajo para la vista actual)
--
CREATE TABLE `v_tipolider` (
`idTipolider` tinyint(3)
,`Nombre` varchar(20)
);
...
--
-- Estructura para la vista `v_color`
--
DROP TABLE IF EXISTS `v_color`;

CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `v_color`  AS  select `color`.`idColor` AS `idColor`,`color`.`Nombre` AS `Nombre` from `color` ;

-- --------------------------------------------------------

--
-- Estructura para la vista `v_denominacion`
--
DROP TABLE IF EXISTS `v_denominacion`;

CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `v_denominacion`  AS  select `denominacion`.`idDeno` AS `idDeno`,`denominacion`.`Nombre` AS `Nombre` from `denominacion` ;

-- --------------------------------------------------------

--
-- Estructura para la vista `v_estado`
--
DROP TABLE IF EXISTS `v_estado`;

CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `v_estado`  AS  select `estado`.`idEstado` AS `idEstado`,`estado`.`Nombre` AS `Nombre` from `estado` ;

-- --------------------------------------------------------

--
-- Estructura para la vista `v_pais`
--
DROP TABLE IF EXISTS `v_pais`;

CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `v_pais`  AS  select `pais`.`idPais` AS `idPais`,`pais`.`Nombre` AS `Nombre` from `pais` ;

-- --------------------------------------------------------

--
-- Estructura para la vista `v_parentesco`
--
DROP TABLE IF EXISTS `v_parentesco`;

CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `v_parentesco`  AS  select `parentesco`.`idparentesco` AS `idparentesco`,`parentesco`.`nombre` AS `nombre` from `parentesco` ;

-- --------------------------------------------------------

--
-- Estructura para la vista `v_rango`
--
DROP TABLE IF EXISTS `v_rango`;

CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `v_rango`  AS  select `rango`.`idRango` AS `idRango`,`rango`.`Nombre` AS `Nombre` from `rango` ;

-- --------------------------------------------------------

--
-- Estructura para la vista `v_sexo`
--
DROP TABLE IF EXISTS `v_sexo`;

CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `v_sexo`  AS  select `sexo`.`idSexo` AS `idSexo`,`sexo`.`Nombre` AS `Nombre` from `sexo` ;

-- --------------------------------------------------------

--
-- Estructura para la vista `v_tipodoc`
--
DROP TABLE IF EXISTS `v_tipodoc`;

CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `v_tipodoc`  AS  select `tipodoc`.`idTipoDoc` AS `idTipoDoc`,`tipodoc`.`Nombre` AS `Nombre` from `tipodoc` ;

-- --------------------------------------------------------

--
-- Estructura para la vista `v_tipolider`
--
DROP TABLE IF EXISTS `v_tipolider`;

CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `v_tipolider`  AS  select `tipolider`.`idTipolider` AS `idTipolider`,`tipolider`.`Nombre` AS `Nombre` from `tipolider` ;
...
--
-- AUTO_INCREMENT de la tabla `puntajejuegos`
--
ALTER TABLE `puntajejuegos`
...
--
ALTER TABLE `acudiente`
  ADD CONSTRAINT `acudiente_ibfk_1` FOREIGN KEY (`nDoc`) REFERENCES `persona` (`nDoc`) ON UPDATE CASCADE;
...
--
ALTER TABLE `detalleregistro`
  ADD CONSTRAINT `detalleregistro_ibfk_1` FOREIGN KEY (`idEstLid`) REFERENCES `lider` (`nDoc`) ON UPDATE CASCADE,
...
--
ALTER TABLE `perfil`
  ADD CONSTRAINT `perfil_ibfk_1` FOREIGN KEY (`idEstLid`) REFERENCES `lider` (`nDoc`) ON UPDATE CASCADE;
...
--
ALTER TABLE `puntajejuegos`
  ADD CONSTRAINT `puntajejuegos_ibfk_1` FOREIGN KEY (`iglesia`) REFERENCES `iglesia` (`idIglesia`) ON UPDATE CASCADE;
